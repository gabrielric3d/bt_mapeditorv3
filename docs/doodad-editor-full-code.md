# Doodad Editor - Full Source Code

This document contains all the code added to implement the Doodad Editor GUI for Remere's Map Editor.

## Table of Contents

1. [New Files](#new-files)
   - [doodad_editor_window.h](#doodad_editor_windowh)
   - [doodad_editor_window.cpp](#doodad_editor_windowcpp)
2. [Modified Files](#modified-files)
   - [doodad_brush.h](#doodad_brushh)
   - [doodad_brush.cpp](#doodad_brushcpp)
   - [palette_brushlist.h](#palette_brushlisth)
   - [palette_brushlist.cpp](#palette_brushlistcpp)
3. [Build Files](#build-files)
   - [CMakeLists.txt](#cmakeliststxt)
   - [RME.vcxproj](#rmevcxproj)
   - [RME.vcxproj.filters](#rmevcxprojfilters)

---

## New Files

### doodad_editor_window.h

Create this file at `source/doodad_editor_window.h`:
...
#ifndef RME_DOODAD_EDITOR_WINDOW_H_
#define RME_DOODAD_EDITOR_WINDOW_H_
...
#endif // RME_DOODAD_EDITOR_WINDOW_H_
```

---

### doodad_editor_window.cpp

Create this file at `source/doodad_editor_window.cpp`:
    ...
    for (const auto& pair : brushMap) {
        ...
        if (brush && brush->isDoodad()) {
            ...
            if (doodad) {
                ...
                info.compositeCount = doodad->getCompositeCount(0);
                info.singleCount = doodad->getSingleCount(0);
...
void DoodadEditorDialog::LoadDoodadBrush(const wxString& brushName) {
    ...
    if (doodad->getMaxVariation() > 0) {
        // Load all single items
        int singleCount = doodad->getSingleCount(0);
        for (int i = 0; i < singleCount; i++) {
            uint16_t itemId = doodad->getSingleItemId(0, i);
            int chance = doodad->getSingleItemChance(0, i);
            if (itemId > 0) {
        ...
        int compositeCount = doodad->getCompositeCount(0);
        for (int i = 0; i < compositeCount; i++) {
            const CompositeTileList& composite = doodad->getCompositeAt(0, i);
            if (!composite.empty()) {
                DoodadComposite comp;
                comp.chance = doodad->getCompositeChanceAt(0, i);

                for (const auto& pair : composite) {
                    const Position& pos = pair.first;
                    const ItemVector& items = pair.second;
                    for (Item* item : items) {
    ...
    for (int x = 0; x < DOODAD_GRID_SIZE; x++) {
        for (int y = 0; y < DOODAD_GRID_SIZE; y++) {
            ...
            if (itemId > 0) {
                ...
                if (type.id != 0) {
                    sprite = g_gui.gfx.getSprite(type.clientID);
                }
                if (sprite) {
                    sprite->DrawTo(&dc, SPRITE_SIZE_32x32, px + 3, py + 3, 32, 32);
                } else {
...
void DoodadPreviewPanel::OnPaint(wxPaintEvent& event) {
    ...
    for (const auto& item : m_items) {
        ...
        if (sprite) {
            sprite->DrawTo(&dc, SPRITE_SIZE_32x32, px, py, cellSize, cellSize);
        } else {
...
Add these method declarations in the `public` section of the `DoodadBrush` class:
...
Add these method implementations at the end of the file:
...
Add this method declaration in the `BrushPalettePanel` class:
...
**2. Add constant in the anonymous namespace:**

```cpp
constexpr int kCreateDoodadButtonId = wxID_HIGHEST + 4101;
```

**3. Add event handler in the event table:**

```cpp
EVT_BUTTON(kCreateDoodadButtonId, BrushPalettePanel::OnClickCreateDoodad)
```

**4. Add button creation in the constructor (after the border button creation):**
...
**5. Add the event handler implementation:**
...
Add these lines to `source/CMakeLists.txt`:
...
Add these lines inside the appropriate `<ItemGroup>`:
...
Add these lines in the appropriate filter groups:
...
This code is released under the GNU General Public License v3.0, consistent with Remere's Map Editor.

---

## Usage

1. Open RME and load your client data
2. Go to the **Doodad** palette
3. Click the **"Create Doodad"** button
4. Use the left panel to browse existing doodads or create new ones
5. Configure properties, single items, and composites
6. Click **"Save to Clipboard"** to generate the XML
7. Paste the XML into your `doodads.xml` file
