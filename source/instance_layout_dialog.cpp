...
private:
	...
	static void DrawCreatureSprite(wxDC& dc, const wxRect& rect, int lookType)
	{
		...
		dc.SetBrush(wxBrush(wxColour(58, 68, 92)));
	...
	void OnPaint(wxPaintEvent&)
	{
		...
		dc.SetBackground(wxBrush(wxColour(14, 19, 30)));
		dc.Clear();
		...
		dc.SetPen(wxPen(wxColour(48, 64, 94), 1));
		dc.SetBrush(wxBrush(wxColour(18, 25, 38)));
		dc.DrawRoundedRectangle(panelRect, 6);

		dc.SetTextForeground(wxColour(220, 228, 241));
		...
		dc.SetPen(wxPen(wxColour(83, 122, 175), 2));
		dc.SetBrush(wxBrush(wxColour(24, 35, 53)));
		dc.DrawRectangle(areaRect);

		dc.SetPen(wxPen(wxColour(120, 160, 220), 1, wxPENSTYLE_DOT));
		dc.DrawLine(areaRect.GetLeft(), areaRect.GetTop(), areaRect.GetRight(), areaRect.GetBottom());

		dc.SetBrush(wxBrush(wxColour(141, 196, 255)));
		dc.SetPen(*wxTRANSPARENT_PEN);
		dc.DrawCircle(areaRect.GetTopLeft(), 3);

		dc.SetBrush(wxBrush(wxColour(255, 195, 140)));
		dc.DrawCircle(areaRect.GetBottomRight(), 3);

		dc.SetTextForeground(wxColour(184, 202, 226));
...
bool InstanceLayoutDialog::ApplyInstancesToMap(
	...
	std::vector<Position> sourceTiles;
		...
		for(int y = sourceArea.from.y; y <= sourceArea.to.y; ++y) {
			for(int x = sourceArea.from.x; x <= sourceArea.to.x; ++x) {
				if(map.getTile(x, y, z)) {
					sourceTiles.push_back(Position(x, y, z));
				}
			}
		}
	}
	...
	BatchAction* batch = editor->createBatch(ACTION_PASTE_TILES);
	std::vector<Position> changedPositions;
	changedPositions.reserve(sourceTiles.size() * copyTargets.size());

	for(const InstancePlacement* placement : copyTargets) {
		Action* action = editor->createAction(batch);
		int placementChanges = 0;

		for(const Position& sourcePos : sourceTiles) {
			...
			action->addChange(newd Change(newDestinationTile));
	...
	if(g_settings.getInteger(Config::USE_AUTOMAGIC) && g_settings.getInteger(Config::BORDERIZE_PASTE)) {
		Action* borderAction = editor->createAction(batch);
		...
		for(Tile* tile : borderTiles) {
			...
			newTile->wallize(&map);
			borderAction->addChange(newd Change(newTile));
		}

		batch->addAndCommitAction(borderAction);
	}
