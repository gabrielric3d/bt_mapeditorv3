...
BEGIN_EVENT_TABLE(BrowseTileWindow, wxDialog)
	...
	EVT_BUTTON(wxID_MOVEUP, BrowseTileWindow::OnClickMoveUp)
	EVT_BUTTON(wxID_MOVEDOWN, BrowseTileWindow::OnClickMoveDown)
END_EVENT_TABLE()
...
wxDialog(parent, wxID_ANY, "Browse Field", position, wxSize(600, 400), wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER)
{
	...
	moveup_button = newd wxButton(this, wxID_MOVEUP, "Move Up");
	moveup_button->Enable(false);
	buttons->Add(moveup_button);
	buttons->AddSpacer(5);
	movedown_button = newd wxButton(this, wxID_MOVEDOWN, "Move Down");
	...
	infoSizer->Add(newd wxStaticText(this, wxID_ANY, "World Boss zone:  " + b2yn(tile->getMapFlags() & TILESTATE_WORLDBOSS)), wxSizerFlags(0).Left());
