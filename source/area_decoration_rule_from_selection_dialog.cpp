...
void ZOrderLayerPanel::CreateControls(const wxColour& headerColor) {
	...
	wxBoxSizer* headerSizer = newd wxBoxSizer(wxHORIZONTAL);
	wxStaticText* headerLabel = newd wxStaticText(headerPanel, wxID_ANY, m_layerName);
	...
	m_itemsListCtrl = newd wxListCtrl(this, wxID_ANY, wxDefaultPosition, wxSize(-1, 120),
	...
	// -- Role radio buttons --
	wxBoxSizer* roleSizer = newd wxBoxSizer(wxHORIZONTAL);
	roleSizer->Add(newd wxStaticText(this, wxID_ANY, "Role:"), 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, 6);
	...
	m_floorClusterRadio = newd wxRadioButton(this, floorRadioId, "Floor Cluster", wxDefaultPosition,
	                                         wxDefaultSize, wxRB_GROUP);
	m_itemsListRadio = newd wxRadioButton(this, itemsRadioId, "Items List");
	...
	// -- Summary text --
	m_summaryText = newd wxStaticText(this, wxID_ANY, "No items");
...
void AreaDecorationRuleFromSelectionDialog::CreateControls() {
	...
	// -- Rule name input --
	wxBoxSizer* nameSizer = newd wxBoxSizer(wxHORIZONTAL);
	nameSizer->Add(newd wxStaticText(this, wxID_ANY, "Rule Name:"), 0,
	               wxALIGN_CENTER_VERTICAL | wxRIGHT, 6);
	m_nameInput = newd wxTextCtrl(this, ID_RULE_NAME, "Selection Rule");
	...
	wxScrolledWindow* scrolledWin = newd wxScrolledWindow(this, wxID_ANY, wxDefaultPosition,
	                                                      wxDefaultSize, wxVSCROLL);
	scrolledWin->SetScrollRate(0, 10);
	wxBoxSizer* scrollSizer = newd wxBoxSizer(wxVERTICAL);

	// Layer 0: Ground (green)
	m_groundPanel = newd ZOrderLayerPanel(scrolledWin, "Ground", 0, wxColour(60, 120, 60));
	m_groundPanel->SetRole(0); // Default: Floor Cluster
	scrollSizer->Add(m_groundPanel, 0, wxEXPAND | wxALL, 2);

	// Layer 1: Borders (blue)
	m_bordersPanel = newd ZOrderLayerPanel(scrolledWin, "Borders", 1, wxColour(60, 80, 140));
	...
	m_bottomPanel = newd ZOrderLayerPanel(scrolledWin, "Bottom Items", 2, wxColour(160, 100, 40));
	m_bottomPanel->SetRole(1); // Default: Items List
	scrollSizer->Add(m_bottomPanel, 0, wxEXPAND | wxALL, 2);

	// Layer 3: Regular Items (purple)
	m_regularPanel = newd ZOrderLayerPanel(scrolledWin, "Regular Items", 3, wxColour(120, 60, 140));
	...
	// -- Settings section --
	wxStaticBoxSizer* settingsBox = newd wxStaticBoxSizer(wxVERTICAL, this, "Rule Settings");
	wxFlexGridSizer* settingsGrid = newd wxFlexGridSizer(3, 2, 4, 8);
	settingsGrid->AddGrowableCol(1, 1);

	// Instance Count
	settingsGrid->Add(newd wxStaticText(this, wxID_ANY, "Instance Count:"), 0,
	                  wxALIGN_CENTER_VERTICAL);
	m_instanceCountSpin = newd wxSpinCtrl(this, ID_INSTANCE_COUNT, "1", wxDefaultPosition,
	                                      wxSize(80, -1), wxSP_ARROW_KEYS, 1, 100, 1);
	settingsGrid->Add(m_instanceCountSpin, 0);

	// Min Distance
	settingsGrid->Add(newd wxStaticText(this, wxID_ANY, "Min Distance:"), 0,
	                  wxALIGN_CENTER_VERTICAL);
	m_minDistanceSpin = newd wxSpinCtrl(this, ID_MIN_DISTANCE, "5", wxDefaultPosition,
	                                    wxSize(80, -1), wxSP_ARROW_KEYS, 1, 50, 5);
	settingsGrid->Add(m_minDistanceSpin, 0);

	// Density
	settingsGrid->Add(newd wxStaticText(this, wxID_ANY, "Density (%):"), 0,
	                  wxALIGN_CENTER_VERTICAL);
	m_densitySpin = newd wxSpinCtrl(this, ID_DENSITY, "100", wxDefaultPosition,
	...
	// -- Summary section --
	wxStaticBoxSizer* summaryBox = newd wxStaticBoxSizer(wxVERTICAL, this, "Summary");
	m_clusterSummaryText = newd wxStaticText(this, wxID_ANY, "Cluster: 0 tiles");
	m_itemsSummaryText = newd wxStaticText(this, wxID_ANY, "Items: 0 unique items");
	...
	m_previewBtn = newd wxButton(this, ID_PREVIEW_CLUSTER, "Preview Cluster");
...
void AreaDecorationRuleFromSelectionDialog::ExtractItemsFromSelection() {
	...
	Selection& selection = m_editor.getSelection();
...
void AreaDecorationRuleFromSelectionDialog::OnPreviewCluster(wxCommandEvent& event) {
	...
	ClusterPreviewWindow* preview = newd ClusterPreviewWindow(this, m_generatedRule, nullptr, "Cluster Preview");
	preview->ShowModal();
	preview->Destroy();
}
