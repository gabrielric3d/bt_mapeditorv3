...
{
	Map* active_map = targetmap ? targetmap : map;
	if(!active_map) {
		return;
	}
	...
	if(exit.isValid()) {
		Tile* oldexit = active_map->getTile(exit);
		if(oldexit)
	...
	Tile* newexit = active_map->getTile(pos);
	if(!newexit) {
		newexit = active_map->allocator(active_map->createTileL(pos));
		active_map->setTile(pos, newexit);
	}
